...
import B from "@/components/ui/bold";
import Inline_para from "../components/ui/inline_para";
import H2 from "@/components/ui/sub_heading";
          ...
          <div className="container mx-auto px-4 lg:px-8 max-w-4xl">
            ...
            <div className="flex items-center gap-4 mb-4">
              <span className="px-3 py-1 bg-accent text-accent-foreground text-sm font-medium rounded-full">
                {article.find((note) => note.id == id).subject}
              </span>
              <span className="flex items-center gap-1.5 text-neutral-400 text-sm">
                <Clock className="w-4 h-4" />
                {article.find((note) => note.id == id).read}
              </span>
            </div>
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-neutral-500 leading-tight">
              {article.find((note) => note.id == id).title}
            </h1>
          </div>
        </div>
      </section>
              ...
              {article
                ...
                .map((block, idx) => {
                  ...
                  if (block.startsWith("**") && block.includes(":**")) {
                    const arrText = block.split(":**");
                    return (
                      <>
                        {arrText.map((text, index) => (
                          <>
                            {text.startsWith("**") ? (
                              <B key={index} val={text.replace("**", "")}></B>
                            ) : (
                                  ...
                                  {item && item.startsWith("**") ? (
                                    ...
                                    <>
                                      ...
                                      {item && item.includes("**") ? (
                                        ...
                                        <Inline_para key={index} val={item} />
                                      )}
                                    </>
                                  )}
                                </>
